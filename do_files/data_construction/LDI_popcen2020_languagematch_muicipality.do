...
* Count number of speakers per language per Municipality
gen one = 1
drop if lang==""
...
collapse (sum) one (first) TagSim , by(birthmun lang)
rename one num_speakers // Total of per-province specific lang speakers

preserve
collapse (sum) num_speakers, by(birthmun)
rename num_speakers munpop_control

tempfile mun_pop
save `mun_pop'
restore

merge m:1 birthmun using `mun_pop', nogenerate
gen share = num_speakers / munpop_control
encode lang, gen(lang_id)
* NOTE: A line here previously read `replace TagSim=1 if lang=="Japanese"`,
* incorrectly forcing perfect Tagalog similarity for Japanese speakers
* (likely a copy-paste artifact). Removed in the replication version;
* Japanese speakers now retain their correct (low) similarity score from
* the LangSimilarity_Tagalog.dta lookup table merged in above. Japanese
* speakers are a minimal share of the sample, so this has negligible
...
*** RESHAPING ***
keep birthmun lang_id LDItag
reshape wide LDItag, i(birthmun) j(lang_id)


egen LDItag_mun=rowtotal(LDItag1-LDItag63)

gen LDIshift_mun=1-LDItag_mun

lab var LDItag_mun "Tagalog Ling. Distance Index (Municipality)"
lab var LDIshift_mun "LDI Difference (pre-post)"

keep birthmun LDItag_mun LDIshift_mun

save $ldi/LDI_Tagalog_municipality, replace
restore
